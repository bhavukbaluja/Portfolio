...
var NodeTypingsInstaller = class extends typescript_exports.server.typingsInstaller.TypingsInstaller {
  ...
  installWorker(requestId, packageNames, cwd, onRequestCompleted) {
    ...
    const hasError = typescript_exports.server.typingsInstaller.installNpmPackages(this.npmPath, typescript_exports.version, packageNames, command => this.execSyncAndLog(command, {
      cwd
    }));
    if (this.log.isEnabled()) {
...
process.on("message", req => {
  installer ?? (installer = new NodeTypingsInstaller(globalTypingsCacheLocation, typingSafeListLocation, typesMapLocation, npmLocation, validateDefaultNpmLocation, /*throttleLimit*/
  5, log));
