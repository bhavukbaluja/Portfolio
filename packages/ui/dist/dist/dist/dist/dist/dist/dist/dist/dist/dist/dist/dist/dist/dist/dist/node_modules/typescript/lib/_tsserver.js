...
`;
    ...
    write(s, _type) {
      if (this.fd >= 0) {
        ...
        import_fs.default.writeSync(this.fd, buf, 0, buf.length, /*position*/
        null);
      }
  ...
  let canWrite = true;
  if (useWatchGuard) {
    const currentDrive = extractWatchDirectoryCacheKey(sys4.resolvePath(sys4.getCurrentDirectory()), /*currentDriveKey*/
    void 0);
...
function startNodeSession(options, logger, cancellationToken) {
  ...
  const _NodeTypingsInstallerAdapter = class _NodeTypingsInstallerAdapter extends typescript_exports.server.TypingsInstallerAdapter {
    constructor(telemetryEnabled2, logger2, host, globalTypingsCacheLocation, typingSafeListLocation2, typesMapLocation2, npmLocation2, validateDefaultNpmLocation2, event) {
      super(telemetryEnabled2, logger2, host, globalTypingsCacheLocation, event, _NodeTypingsInstallerAdapter.maxActiveRequestCount);
    ...
    createInstallerProcess() {
      ...
      this.host.setImmediate(() => this.event({
        pid: this.installer.pid
      }, "typingsInstallerPid"));
      process.on("exit", () => {
        this.installer.kill();
      });
  ...
  class IOSession extends typescript_exports.server.Session {
    constructor() {
      ...
      if (this.canUseEvents && this.eventPort) {
        const s = import_net.default.connect({
          port: this.eventPort
        }, () => {
          this.eventSocket = s;
