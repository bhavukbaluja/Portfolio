...
import { QuotaUsage_URL } from '@utils/Config/URLs'; // 👈 Update the API path
import Literal from "@ui/literals";
import { LanguageContext } from '@ui/literals/LanguageProvider';

const QuotaUsageGrid = forwardRef((props, ref) => {
  ...
  const [columnVisibilityModel, setColumnVisibilityModel] = useState({id: false, oldValue: false, newValue: false});

  const columns = [
    { field: "id", headerName: Literal[lang].id, flex: 0.5, sortable: true },
    { field: "date", headerName: Literal[lang].date, flex: 1.5, sortable: true, renderCell: (params)=>getModifiedDateCell(params, "date", "date") },
    { field: "brevoCount", headerName: Literal[lang].brevo +" "+ Literal[lang].count, flex: 0.5, sortable: true },
    { field: "brevoQuota", headerName: Literal[lang].brevo +" "+ Literal[lang].quota, flex: 0.5, sortable: true },
    { field: "gmailCount", headerName: Literal[lang].gmail +" "+ Literal[lang].count, flex: 1, sortable: true },
    { field: "smsCount", headerName: Literal[lang].sms +" "+ Literal[lang].count, flex: 1, sortable: true },
    { field: "whatsappCount", headerName: Literal[lang].whatsapp +" "+ Literal[lang].count, flex: 1, sortable: true },
    // { field: "description", headerName: Literal[lang].description, flex: 1.4, sortable: true },
    // { field: "performedBy", headerName: Literal[lang].performedBy, flex: 1, sortable: true},
    // { field: "timestamp", headerName: Literal[lang].timestamp, flex: 1.0, sortable: true},
  ];
  ...
  const getUrl = (paginationObj, searchParam) => {
    ...
    let url = QuotaUsage_URL + '/all'+'?advancesearch=' + encodeURIComponent('archiveFlag=' + archiveFlag + '$isAdvanceSearch=false');
    // let url = QuotaUsage_URL + '/tracking/order/BSH-1753372977053-I1U8GE'+'?advancesearch=' + encodeURIComponent('archiveFlag=' + archiveFlag + '$isAdvanceSearch=false');
