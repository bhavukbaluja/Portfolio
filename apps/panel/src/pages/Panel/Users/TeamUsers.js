...
const TeamUsers = ({ loading, setLoading, showSnackBar, setImageRefreshKey, isMobile, imageRefreshKey }) => {

    const { lang } = useContext(LanguageContext);
    let entity = "teamUsers";
    ...
    const clickAddTeamUsers = () => setOpenAdd(true);
    ...
    // --- Action Handlers (Write to pendingActionRef) ---

    const clickDelete = async (row) => {
        if (row?.status === "ACTIVE") {
            setChildren([
                Literal[lang].activeCantAction.replace("{entity}", Literal[lang].team+" "+Literal[lang].member).replace("{action}", Literal[lang].deleted),
                Literal[lang].deactivateFirst.replace("{entity}", Literal[lang].team+" "+Literal[lang].member),
            ]);
            setAlertMsg(Literal[lang].entityIsActive.replace("{0}", row?.firstName+" "+row?.lastName).replace("{entity}", Literal[lang].team+" "+Literal[lang].member));
            setOpenAlert(true);
        } else {
            pendingActionRef.current = { type: "DELETE", payload: row };
            setConfirmMsg(Literal[lang].deleteMsg.replace("{0}", row?.firstName+" "+row?.lastName).replace("{1}", Literal[lang].team+" "+Literal[lang].member));
    ...
    const clickResetPassword = async (row) => {
        ...
        setConfirmMsg(Literal[lang].confirmMsg.replace("{0}", row?.firstName+" "+row?.lastName).replace("{1}", Literal[lang].team+" "+Literal[lang].member+"'s account").replace("{2}", Literal[lang].resetToDefaultPassword));
        setConfirmBoxOpen(true);
    };

    const clickDiscontinued = async (row) => {
        pendingActionRef.current = { type: "DISCONTINUE", payload: row };
        setConfirmMsg(Literal[lang].confirmMsg.replace("{0}", row?.firstName+" "+row?.lastName).replace("{1}", Literal[lang].team+" "+Literal[lang].member+"'s account").replace("{2}", Literal[lang].setAsDiscontinued));
        setConfirmBoxOpen(true);
    };

    const closeConfirmBox = () => {
        setConfirmBoxOpen(false);
        // Do NOT resetData here. Wait for action or next click.
    };
    ...
    const handleConfirmYes = async () => {
        ...
        if (type === "DELETE") {
            ...
            // ✅ Added logic for when UsersGrid requests activation
            url = Users_URL + "/updateStatus";
    ...
    const content = (
        <BaseAddButton
            handleClick={clickAddTeamUsers}
            addMsg={Literal[lang].add + " " + Literal[lang].teamUsers+ " " + Literal[lang].member}
    ...
    return (
        <div className="panel-main">
            ...
            <UsersGrid
                ...
                actionRef={pendingActionRef} // 2. Action Data Ref (Passed as Prop)
