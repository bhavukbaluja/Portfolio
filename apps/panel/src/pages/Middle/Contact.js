...
import BaseTextField2 from "@ui/components/UI/fields/BaseTextField2"; // Ensure path is correct
import Literal from "@ui/literals";
import { LanguageContext } from '@ui/literals/LanguageProvider';
import propertiesData from "@utils/Config/Properties.json";
import { WhatsApp_URL } from "@utils/Config/URLs";
...
const Contact = ({ isMobile }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Logic to send email/data goes here
    console.log("Form Submitted", formData);
    alert(Literal[lang].messageSent || "Message Sent");
    ...
    <section id="contact">
      ...
      <div style={{display: 'flex', flexDirection: isMobile? 'column': 'row', gap: '20px'}} data-aos="fade-up" data-aos-delay="50">          
          {/* Left Column: Contact Info Tiles */}
            <Box style={{ display: 'flex', flexDirection: 'column', gap: '20px', flex: 2, padding: '20px', borderRadius: '10px', border: '1px solid var(--color-gray-300)', boxShadow: '0 5px 20px var(--color-gray-400)'}}>
              ...
              <ContactDetailsTile
                ...
                icon={<WhatsAppIcon />}
                title={"whatsAppUs"}
                body={properties?.user?.mobile} // Corrected: Using actual number property
                url={WhatsApp_URL}
              />
              <ContactDetailsTile
                isMobile={isMobile}
          ...
          {/* Right Column: Contact Form */}
            ...
            <Box sx={{ display: "flex", flexWrap: "wrap", alignItems: "center", width: '100%', gap: 2, flex: 4, padding: '20px', borderRadius: '10px', border: '1px solid var(--color-gray-300)', boxShadow: '0 5px 20px var(--color-gray-400)' }}>                
                {/* Name */}
